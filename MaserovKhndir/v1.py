...
class segments(Scene):
    def construct(self):
        ...
        b_2 = np.array([3, -1, 0])    # second segment's ending coordinare
        c_2 = a_2 + b_1 - a_1         # extra part of the second segment is (c2, b2) 
        ...
        cal3_8 = MathTex("8").move_to([4.7, 0, 0])                         # 6+2 = 8
        ...
        dashed_line = DashedLine(b_1, c_2)
        segment_difference = Line(c_2, b_2).set_color(RED)

        brace_difference = BraceBetweenPoints(c_2, b_2)
        ...
        brace_2 = BraceBetweenPoints(a_2, c_2)
        ...
        self.play(Create(segment_1), Create(startpoint_1))  # draw Hayk's segment
        self.add(endpoint_1)
        self.wait(0.5)

        self.play(Write(name_2))                            # write second name - Mane

        self.play(Create(segment_2), Create(startpoint_2))  # draw Mane's segment
        self.add(endpoint_2)
        self.wait(0.5)
        ...
        self.play(Create(dashed_line))                      # draw dashed segment
        self.add(extra_point)                               # add a point on Mane's segment
        self.wait(1)

        self.play(Create(segment_difference))               # color difference red
        self.add(extra_point, endpoint_2)
        ...
        self.play(Transform(brace_combined_text.copy(), cal1_14), run_time=1.0)      # 14 - 2 = 12
