...
class Problem11163(Scene):
    def construct(self):
        ...
        a = MathTex(r'\frac{1}{3}')

        task = Task(
                MathTex(r'\#11163', font_size=35), 
                MathTex(
                    r"\textrm{Մեքենագրուհին օրվա առաջին կեսում հավաքեց տեքստի } \frac{1}{3}  \textrm{ մասը,}",
                    r"\textrm{իսկ օրվա երկրորդ կեսում } 10 \textrm{ էջ: Արդյունքում նա հավաքեց ամբողջ տեքստի}", 
                    r"\textrm{ուղիղ կեսը: Քանի՞ էջանոց տեքստ պետք է հավաքի մեքենագրուհին:}",
        ...
        self.play(FadeOut(task))

        self.play(FadeIn(papers.papers), run_time=3)
...
class test2(Scene):
    def construct(self):
        ...
        self.play(Create(p.papers))
        self.play(FadeOut(p.papers))
