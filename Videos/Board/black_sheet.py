...
class outline_experiment(Scene):
	def construct(self):
		...
		for i in range(8):
			self.play(Create(b[i]))
		self.play(board.cells[0][1].animate.set_fill(GREEN, opacity=0.5))
		self.play(
			Wiggle(b[1], rotation_angle=PI/8),
			Wiggle(b[2], rotation_angle=PI/8)
			)
		self.play(ReplacementTransform(b, a))
		#self.play(ReplacementTransform(a, c))
		self.play(board.cells[1][1].animate.set_fill(GREEN, opacity=0.5))
		# self.play(
		# 	FadeTransform(a[2:5], board.outline([board.vertices[1][1], board.vertices[2][1]])),
		#  	# ReplacementTransform(a[3], board.outline([board.vertices[1][1], board.vertices[2][1]])),
		#  	# ReplacementTransform(a[4], board.outline([board.vertices[1][1], board.vertices[2][1]]))
		# )
		self.play(Uncreate(a[4:1:-1]))
		self.play(Create(board.outline([board.vertices[1][1], board.vertices[2][1]])))
		# self.play(
		# 	ClockwiseTransform(a[4:1:-1], board.outline([board.vertices[1][1], board.vertices[2][1]]))
		# )
		# v.remove(board.vertices[1][2].all())
		# v.remove(board.vertices[2][2].all())
		# self.play(Create(board.outline(a)))
